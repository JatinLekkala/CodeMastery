...
const ProblemDetail = ({ theme }) => {
  ...
  useEffect(() => {
    ...
    const handleSubmissionVerdict = (payload) => {
      ...
      if (activeSubmissionIdRef.current === payload.submissionId) {
        ...
        // Clear high-precision stopwatch on completion
        if (timerRef.current) {
  ...
  const handleSubmit = async () => {
    ...
    // Reset progress and start high-resolution stopwatch
    setElapsedTime(null);
    setElapsedMs(0.000);
    setProgressCaseIndex(0);
    setProgressTotalCases(0);
                ...
                >
                  {submitting && actionType === 'submit' ? (
                    <>
                      ...
                      <span>Submitting ({elapsedMs.toFixed(3)}s)...</span>
                    </>
                  ) : (
                    <>
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', fontWeight: 600 }}>
                    <span style={{ color: 'var(--text-secondary)' }}>
                      {actionType === 'submit' ? 'Running test cases...' : 'Executing sandbox run...'}
                    </span>
                    <span style={{ color: 'var(--primary)', fontFamily: 'var(--font-mono)' }}>
                      {progressTotalCases > 0 ? `${progressCaseIndex} / ${progressTotalCases}` : 'Evaluating...'}
                    </span>
                  </div>
                  <div style={{ height: '6px', background: 'rgba(255,255,255,0.05)', borderRadius: '3px', overflow: 'hidden' }}>
                    <div 
                      style={{ 
                        height: '100%', 
                        width: `${progressTotalCases > 0 ? progressPercent : 20}%`, 
